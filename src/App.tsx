            ...
            const renderSepaArcade = (title: string, numbers: string[], outerSites: PeriodontalSiteKey[], innerSites: PeriodontalSiteKey[], outerLabel: string, innerLabel: string) => (
              <section className="sepa-arcade" key={title}>
                ...
                <div className="sepa-sheet" style={{ '--sepa-columns': numbers.length } as CSSProperties}>
                  ...
                  <div className="sepa-label multi">PS {outerLabel}</div>{numbers.map((number) => <button type="button" key={`ps1-${number}`} className="sepa-cell sepa-triplet sepa-tooth-select sepa-depth-values" onClick={() => setSelectedPeriodontalTooth(number)}>{outerSites.map((site) => <span key={site} className={Number(periodontalTeeth[number].sites[site].probingDepth) > 3 ? 'sepa-pathologic' : ''}>{periodontalTeeth[number].sites[site].probingDepth || '·'}</span>)}</button>)}
