        ...
        <main className="main-panel">
          ...
          {activeView === 'periodontogram' && (() => {
            ...
            const renderToothDiagram = (number: string, sites: PeriodontalSiteKey[], orientation: 'upper' | 'lower') => {
              ...
              return <button type="button" key={`${orientation}-${sites[0]}-${number}`} className={`sepa-tooth-number sepa-diagram-cell ${selectedPeriodontalTooth === number ? 'active' : ''} ${absentTeeth.includes(number) ? 'absent' : ''} ${tooth.implant ? 'implant' : ''}`} onClick={() => setSelectedPeriodontalTooth(number)}><svg viewBox="0 0 40 56" aria-label={`${tooth.implant ? 'Implante' : 'Diente'} ${number}, ${orientation === 'upper' ? 'arcada superior' : 'arcada inferior'}`}><defs><linearGradient id={`${illustrationId}-enamel`} x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#ffffff" /><stop offset=".45" stopColor="#fffdf7" /><stop offset="1" stopColor="#d8cdb7" /></linearGradient><linearGradient id={`${illustrationId}-root`} x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#e1d4bc" /><stop offset=".5" stopColor="#fffef9" /><stop offset="1" stopColor="#cfc1a7" /></linearGradient><linearGradient id={`${illustrationId}-implant`} x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#78939b" /><stop offset=".45" stopColor="#eff6f7" /><stop offset="1" stopColor="#66838b" /></linearGradient></defs><g transform={transform}>{tooth.implant ? <><path className="implant-crown" d="M8.2 5.2 Q20 0.7 31.8 5.2 L29.3 15.2 H10.7Z" /><path className="implant-body" fill={`url(#${illustrationId}-implant)`} d="M12.5 15.8 H27.5 L25.3 51.2 Q20 54.5 14.7 51.2Z" /><path className="implant-thread" d="M13.5 21.5 H26.5 M13.3 27.5 H26.7 M14 33.5 H26 M14.4 39.5 H25.6 M15.2 45.5 H24.8" /></> : <><path className={`sepa-tooth-root anatomy-${anatomy}`} fill={`url(#${illustrationId}-root)`} d={rootPath} /><path className={`sepa-tooth-root-detail anatomy-${anatomy}`} d={rootDetail} /><path className={`sepa-tooth-crown anatomy-${anatomy}`} fill={`url(#${illustrationId}-enamel)`} d={crownPath} /><path className="sepa-tooth-highlight" d="M9 10 C12 7 15 7 17 8" /></>}<polyline className="sepa-margin-line" points={marginPoints} /><polyline className="sepa-depth-line" points={depthPoints} /></g></svg><span>{absentTeeth.includes(number) ? '×' : number}</span></button>
            }
