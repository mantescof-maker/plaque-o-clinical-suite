...
function App() {
  ...
  const toggleAllToothSites = (toothNumber: string, field: 'bleeding' | 'suppuration' | 'plaque') => {
    setPeriodontalTeeth((current) => {
      const tooth = current[toothNumber]
      const activate = !Object.values(tooth.sites).every((site) => site[field])
      return { ...current, [toothNumber]: { ...tooth, sites: Object.fromEntries(periodontalSiteKeys.map((site) => [site, { ...tooth.sites[site], [field]: activate }])) as Record<PeriodontalSiteKey, PeriodontalSiteState> } }
    })
  }
            ...
            const renderSepaArcade = (title: string, numbers: string[], outerSites: PeriodontalSiteKey[], innerSites: PeriodontalSiteKey[], outerLabel: string, innerLabel: string) => (
              <section className="sepa-arcade" key={title}>
                ...
                <div className="sepa-sheet" style={{ '--sepa-columns': numbers.length } as CSSProperties}>
                  ...
                  <div className="sepa-label">SG</div>{numbers.map((number) => <button type="button" key={`sg-${number}`} className={`sepa-cell sepa-tooth-select ${Object.values(periodontalTeeth[number].sites).some((site) => site.bleeding) ? 'sepa-on sepa-bleeding' : ''}`} title="Clic: activar o retirar sangrado en los seis sitios" onClick={() => toggleAllToothSites(number, 'bleeding')}>{Object.values(periodontalTeeth[number].sites).filter((site) => site.bleeding).length || '·'}</button>)}
                  <div className="sepa-label">SP</div>{numbers.map((number) => <button type="button" key={`sp-${number}`} className={`sepa-cell sepa-tooth-select ${Object.values(periodontalTeeth[number].sites).some((site) => site.suppuration) ? 'sepa-on sepa-suppuration' : ''}`} title="Clic: activar o retirar supuración en los seis sitios" onClick={() => toggleAllToothSites(number, 'suppuration')}>{Object.values(periodontalTeeth[number].sites).filter((site) => site.suppuration).length || '·'}</button>)}
                  <div className="sepa-label">PL</div>{numbers.map((number) => <button type="button" key={`pl-${number}`} className={`sepa-cell sepa-tooth-select ${Object.values(periodontalTeeth[number].sites).some((site) => site.plaque) ? 'sepa-on sepa-plaque' : ''}`} title="Clic: activar o retirar placa en los seis sitios" onClick={() => toggleAllToothSites(number, 'plaque')}>{Object.values(periodontalTeeth[number].sites).filter((site) => site.plaque).length || '·'}</button>)}
                  ...
                  <div className="sepa-label tooth-row">Diente</div>{numbers.map((number) => { const tooth = periodontalTeeth[number]; const depthPoints = outerSites.map((site, index) => `${5 + index * 15},${20 + Math.min(Number(tooth.sites[site].probingDepth || 0), 8) * 3}`).join(' '); const marginPoints = outerSites.map((site, index) => `${5 + index * 15},${20 + Math.max(Number(tooth.sites[site].recession || 0), -2) * 3}`).join(' '); return <button type="button" key={`tooth-${number}`} className={`sepa-tooth-number ${selectedPeriodontalTooth === number ? 'active' : ''} ${absentTeeth.includes(number) ? 'absent' : ''}`} onClick={() => setSelectedPeriodontalTooth(number)}><svg viewBox="0 0 40 48" aria-hidden="true"><path className="sepa-tooth-outline" d="M8 3 C4 9 7 17 9 23 L12 44 L28 44 L31 23 C33 17 36 9 32 3 C26 7 14 7 8 3Z" /><polyline className="sepa-margin-line" points={marginPoints} /><polyline className="sepa-depth-line" points={depthPoints} /></svg><span>{absentTeeth.includes(number) ? '×' : number}</span></button> })}
